...
*ステータス: Phase 8A 実装済み / polish 進行中*
...
- [ ] ChatGPT: State Referee / Narrator 分離アーキテクチャを設計
- [ ] Grok: `gmBridgeRunner.ts` に二段階ルーティングの最小 prototype を実装
- [ ] 失敗時に current single-stage flow へ戻る fallback を設計
- [ ] clipboard/manual GM workflow でも破綻しない運用案を作る

---

## 🟡 Phase 10: VS Code/Git Native Timeline / シナリオ分岐タイムライン (担当: Claude)
*ステータス: 計画中（Inspector 読み取り UI の prototype あり）*

ゲーム用ワークスペースの Git branch を「並行世界 / セーブ分岐」として扱い、Webview から安全に branch 作成・確認・復帰できるようにする。

- [x] Inspector に Git Timeline ステータス表示（読み取り専用 prototype）
- [x] `gitManager.ts` — 同意ダイアログ付き `git init`、`shell: false` 固定
- [ ] ゲームワークスペースだけを対象にする安全な Git helper を設計
- [ ] Webview に scenario branches 一覧と branch 作成 UI を追加
- [ ] destructive git command を自動実行しない安全策を追加
...
- [ ] `npc_registry.json` に voice profile metadata を追加
- [ ] system/local TTS を優先した provider abstraction を設計
- [ ] Webview の TTS UI と NPC profile を接続
- [ ] 外部API利用時の privacy/security note を追加
