...
## 🟡 Phase 8 (v1.8+): Event-to-Quest & Objective Visualizer (担当: Gemini)
*ステータス: 計画中*

世界シミュレーション（Emergent Simulator）で発生した出来事（食糧危機、派閥対立など）を、プレイヤーが請け負える「クエスト（依頼）」として可視化・接続する。

- [ ] `recentChanges` や派閥・地域イベントから Quest Hook 候補を自動生成
- [ ] NPCの `urgentNeeds` から依頼化
- [ ] Webview (World タブ) に「Quest Board / Quest Hooks」UIを表示
- [ ] Scenario Director へ optional objective 追加
- [ ] 解決後に `world_state` / `npc_registry` へ結果を反映（報酬、好感度変化）
...
LLMの役割分担（State判定用とナラティブ用）による高度なプロンプトアーキテクチャへの進化。

- [ ] **ChatGPT**: 高速モデル(State Patch用)と高品質モデル(情景描写用)を使い分けるアーキテクチャ設計
- [ ] **Grok**: `gmBridgeRunner.ts` における非同期APIルーティング（2段構えの生成）の実装

---

## 🟡 Phase 10: VSCode/Git Native Timeline / シナリオのタイムライン分岐 (担当: Claude)
*ステータス: 計画中*

VS CodeとGitの強みを活かし、シナリオのセーブデータをGitブランチとして管理し、過去に戻って別ルートを試せるようにする機能。

- [ ] Webview上でのブランチ（並行世界）ツリーの視覚的表示
- [ ] Gitコマンド (`git branch`, `git checkout`) を裏で叩くノード拡張とWebviewからの操作インターフェース

---

## 🟡 Phase 11: Adaptive TTS / NPC個別音声 (担当: Grok)
*ステータス: 計画中*

NPCの性格、種族、感情に応じて異なるTTSモデルや声質パラメータを動的に割り当てる音声拡張。

- [ ] 外部TTS API (ElevenLabs等) またはローカルTTSモデルとのPythonスクリプト連携
- [ ] `npc_registry.json` への音声プロファイル情報の追加
