...
## 2026-06-26 (Claude Sonnet 4.6) - Phase 2B: ST ロアブックマッチングエンジン

### 概要
- **新規 `src/lorebookMatcher.ts`**: vscode 非依存の純粋関数 `matchEntriesAgainstText` を分離。ST 互換フィールド (`use_regex`, `secondary_keys`, `insertion_order`) をサポート。
- **`src/gmPromptBuilder.ts`**: `matchLorebookEntries` を新関数の薄いラッパーに置き換え。`LorebookEntry` インターフェースを `lorebookMatcher` から import。
- **`scripts/test_lorebook.js`** (新規): 11 テストケース（Regex/Secondary Keys/ソート/maxEntries 等）。
- **`scripts/validate.js`**: `test_lorebook.js` を `npm test` に統合。
- `CHANGELOG.md` [Unreleased]、`AI_ROADMAP.md` Phase 2B ST ロアブックエンジン → `[x]` に更新。

### Verification
- `npm run compile` 通過（TypeScript エラーなし）
- `npm test` 全通過（test_lorebook.js 含む 11 ケース all OK）

### ST 機能 ↔ 実装状況
| ST 機能 | 実装状況 |
|---------|---------|
| Substring match (OR) | 対応済み（既存）|
| Regex Keys (`use_regex`) | **新規対応** |
| Secondary Keys (AND) | **新規対応** |
| insertion_order ソート | **新規対応** |
| Scan Depth | 呼び出し側で直近 3 エントリ固定（設定値化は将来対応）|
| Exclusion Keys | 将来対応 |
...
- 
pm run compile and 
pm test completed successfully.
