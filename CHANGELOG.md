...
- TF-IDF履歴検索では質問と直後の公開GM回答を一緒に渡し、自己紹介や約束の回答部分が検索から落ちるのを防ぐ。
...
Minor human-test candidate following 1.88.4; no tag or Release published.
...
### Added
- Added the frozen, versioned genre-world preset registry, generation provenance, exact-version reproduction availability checks, exclusion-based canonical-content projection, and an opt-in guaranteed region allocator.

### Changed
- Moved the existing theme-keyed region weights, biome overrides, hazard rules, and region name parts into presets without changing shipped generation behavior.
- Updated the package version to 1.84.31.
...
### Fixed
- Bridge V1-A: PENDING 書き込み失敗時に lifecycle が terminal で固まり再試行不能になる問題を修正（durable 成功まで retry）。
- Bridge V1-A: 検証時の compiled BattleSpec / roster snapshot と hash を sessions に永続化。

### Changed
- パッケージ版を 1.84.21 へ更新（main 1.84.20 の上）。
...
- **Prompt budget / game_rules P1 review fixes** - shared prompt truncation suffix logic now guarantees budget caps without negative-slice overshoot; `loadGameRules()` normalizes persisted JSON against defaults instead of a polluted merged fallback, so invalid strings like `"false"` cannot survive as truthy rule values. Added load-path and budget parity regressions.

- **Settlement viewer polish P2 — time-of-day, water bob, glyph LOD** (Claude, Webview-only) — three follow-ups from the isometric/diorama graphics upgrade. (1) Isometric viewer gets a manual ☀️/🌆/🌙 time-of-day toggle next to the zoom controls (per-settlement, persisted in localStorage): it retints the sky/glow backdrop, vignette, and the extruded blocks' side-face shading + top rim-light color. There is no structured world-clock field in game_state/world_state to drive this automatically (GM-authored `status.time` is free text), so this is a display-only preference rather than a simulation-time readout. (2) Diorama water blocks now bob gently (per-tile phase offset so a pond doesn't pulse in unison) with a matching opacity shimmer, via a small requestAnimationFrame loop that only runs while the Diorama tab is visible, a water block exists, and `prefers-reduced-motion` is off — otherwise water stays static at its resting position/opacity. The loop starts/stops on tab switch (mirrors the existing tile-overmap register/unregister pattern in `85-world.js`) and on scene rebuild/dispose. (3) Isometric tile glyphs (the small letter overlays like `M`/`W`/`#`) are skipped below zoom 0.65 — illegible at that scale on a large, zoomed-to-fit settlement and not worth the per-tile `fillText()` cost. New i18n keys (`webview.world.settlementTimeToggleTitle`, `webview.world.settlementTimeOfDay.{day,dusk,night}`) synced across en/ja/zh-CN/zh-TW. Read-only contract unchanged.

- **Settlement viewer graphics upgrade — isometric & 3D diorama** (Claude, Webview-only) — the two settlement viewers now render like a DF-style visualizer instead of flat schematics. Isometric (`86b-settlement-isometric.js`): tiles are extruded 3D blocks with per-code heights (walls 16px … floor 2px) using the previously-unused left/right face colors, with shaded side faces, ambient-occlusion ground lines, sun-side rim light on top edges, glossy flat water with ripple highlights, marker contact shadows + stems + a golden player halo, a sky-gradient backdrop with an accent glow behind the settlement, and a vignette. Hovered tiles get an accent outline and clicked tiles/markers keep a gold selection outline. Also fixed a latent hit-test bug: hover/click positions are stored in pre-zoom content coordinates but were compared against raw screen coordinates, so clicks drifted at any zoom ≠ 1 — the mouse position is now inverse-transformed around the zoom pivot. Diorama (`86c-settlement-diorama.js`): ACES filmic tone mapping + sRGB output, HemisphereLight (sky/ground bounce from the snapshot palette) instead of flat AmbientLight, subtle dark edge lines per block (capped at 350 blocks), a thicker oversized plinth with a faint survey grid, a larger glowing player cone, and click-to-highlight (gold emissive, restored on deselect/rebuild). Read-only contract unchanged: no payload/schema changes, no state writes; ghost expansion previews and marker fallback lists untouched.

- **Visual Refresh Wave 2 — World / Settlement / Viewer panels** (Claude, CSS-only) — extended `97-visual-refresh.css` to the panels players actually read during play: World tab `<details>` sections become glass cards with accent-bar summaries and themed markers; the map-mode bar becomes a segmented control; market/NPC/quest/petition cards get a unified gradient surface with hover lift and accent glow (active quests keep their amber state); Domain/Guild stat bars and Vehicles fuel/durability bars get accent-gradient fills with glow while preserving the low/empty warning colors; tag/officer/action/module/facility chips become themed pills; Settlement and Diorama detail/expand/toolbar chrome gets the same glass treatment; Observatory market cards, turn chips and chronicle rows, the Status sidebar blocks (condition/inventory/skills as cards, tabular numerals for values), and gallery thumbnail hover are polished to match. All colors run through the existing `--vr-accent-rgb` theme variables so 世界観テーマ switching recolors every panel consistently; `prefers-reduced-motion` disables the new transitions. Runtime-injected `<style>` blocks from `85-world.js` win document-order ties, so overrides use `#pane-world`/`#pane-vehicles`/`#world-observatory` id prefixes for specificity (same for `89/98/99` CSS modules that concatenate after `97`). No JS, HTML, host, or state changes.
...
- **MEDIA-M1 Compatibility Gate + Media Profile Spine** - `mediaProfileCore.ts` / `mediaCompatibility.ts`: preflight validation stack that blocks incompatible checkpoint/workflow/prompt-mode combinations (e.g. an Anima checkpoint on the SDXL-simple graph, the confirmed `clip input is invalid: None` failure) before ComfyUI queue/spawn, across scene, portrait, expression, and world-map cartography generation. `image_gen_config.json` migrates non-destructively from v1 to a v2 envelope with explicit `profileId`/`modelFamily` while retaining sanitized legacy fields; a compatibility rejection never consumes the runtime image circuit-breaker and remains retryable after correcting settings. The Python `comfyui_generate.py` boundary trusts a host-validated `TA_MEDIA_PREFLIGHT=validated` contract and does not re-resolve workspace values that could recreate a different stack. Independently adversarially verified (`docs/ai-tasks/MEDIA-M1-INDEPENDENT-VERIFY.md`, `MEDIA_M1_VERIFY_PASS`).

- **Terminology and event classification contracts** - `docs/TERMINOLOGY_CONTRACT.md` and `docs/EVENT_CLASSIFICATION_GLOSSARY.md` define canonical naming, cross-ledger reference/time vocabulary, and composite semantic event matching rules. `livingWorldTypes.ts` now exposes shared semantic evaluators so commerce, NPC agency, and Debug Trace can avoid severity-only false positives.
...
- **Genesis Mode G2 Webview UI** — `webview/modules/06-genesis-guide.js` + `webview/styles/11-genesis-guide.css`: a staged Genesis Guide wizard (genre → playstyle → pressure → bookkeeping → protagonist mode → image-generation preference) reachable from a new prominent Start Hub hero banner (not just the rocket icon). Live preview mirrors `resolveRulesProfile()` locally (UI-only, non-authoritative) to show enabled systems, summary rows, and warnings before anything is applied. Summary screen shows the genre's `comfyUiStylePrompt` with a copy-to-clipboard button and a "Generate Genesis Image" button placeholder (both non-blocking; no ComfyUI backend wired yet). Guide/background art loads through a `window.__LR_GENESIS_ASSET_BASE_URI__` placeholder the host can substitute later (`{{genesisAssetBaseUri}}` in `webview/index.html`); falls back to an emoji badge until then. Final step only sends `genesisApplyProfile` / `genesisGenerateImage` postMessages — it does not write `game_rules.json` or any canonical state (RP2 host apply gate is still future work). Existing Quickstart and the advanced Game Rules panel are unchanged; "Advanced settings" in the wizard just opens the existing panel. New i18n keys (`webview.genesis.*`) added to en/ja/zh-CN/zh-TW. Fixed a latent `.start-hub` layout bug (`justify-content: center` on an overflowing flex column hid content above the fold with no way to scroll to it) by switching to `flex-start`, so the new hero banner and all other Start Hub content stay reachable regardless of panel height.

### Fixed

- **INSTALLER-RELEASE-001: Antigravity direct-folder fallback false failure** - `scripts/install_vscode_extension.ps1`: the Antigravity direct-folder fallback aggregated per-target results with `return @($results)` where `$results` is a `System.Collections.Generic.List[object]`. On the affected Windows PowerShell 5.1 build, wrapping a `List[object]` with the `@()` array subexpression operator throws `System.ArgumentException: Argument types do not match` even after every folder copy reports `OK`, so a fully successful fallback was still reported as a failed install. Reproduced and confirmed behaviorally (minimal repro throws the exact error; `List[string]`, `ArrayList`, and `.ToArray()` do not). Fixed by returning `$results.ToArray()` instead of `@($results)`. A failed primary CLI path no longer causes the whole installer to fail when the direct-folder fallback actually succeeded; the primary error is still surfaced as a warning.
- **Stale extension version identity** - `package.json`/`package-lock.json` stayed at `1.77.15` through several integrated phases (Antigravity Relay repairs, Start Hub/playtest unblock, Gameplay Slice work, KNOWLEDGE-OPS-001, MEDIA-M1), so every built VSIX and installed extension reported the same version regardless of which integrated build it actually contained. Bumped to `1.78.0` to identify the current MEDIA-M1-integrated build and added a durable versioning rule (`docs/VERSION_TRUTH.md`): patch bump for repair-only release builds, minor bump for backward-compatible feature phases, a human-smoke candidate integrated to main must carry a version newer than the previously shipped/tested candidate, and docs-only commits do not require a bump.
...
- **world_state parse warning buffer** — `loadWorldState()` clears stale cap warnings on missing/no-workspace/parse-failure paths, restores cached warnings on cache hits, and updates cache identity when `world_state.json` is absent; `scripts/test_world_state_warning_buffer.js`.

### Added

- **Debug Trace P1 design** — `docs/DEBUG_TRACE_P1_DESIGN.md` (Codex): causal world-debug trace vocabulary gate.
- **Debug Trace P1 core** — `debugTraceCore.ts`: bounded ring buffer, linkage warnings, audience projection; `scripts/test_debug_trace_core.js`. No runtime wiring.
...
- **State Orchestrator SO2 Transaction Planning Gate design** - added `docs/STATE_ORCHESTRATOR_SO2_TRANSACTION_PLANNING_GATE.md`, defining the next State Orchestrator step as a pure, read-only transaction planning report. SO2 describes planned GM-turn ledger steps from `TURN_LEDGER_PERSIST_ORDER` and SO1 descriptors without writing files, changing queues, executing rollback, modifying `statePatch`/`TurnResult`, or wiring Webview/Remote/Replay/GM prompt behavior.

- **State Orchestrator SO2 Transaction Planning Gate** — `stateOrchestratorPlanCore.ts`: pure `buildStateTransactionPlan` for `gm_turn` requests using SO1 `LEDGER_DESCRIPTORS` + `TURN_LEDGER_PERSIST_ORDER`; reports planned/skipped/blocked steps, descriptor-backed failure policies, out-of-scope ledger ids, and bounded warnings. `scripts/test_state_orchestrator_plan_core.js`.

- **Context Engine P0 Inspector design** - added `docs/CONTEXT_ENGINE_P0_INSPECTOR_DESIGN.md`, defining the first Context Engine entry point as a read-only extension of the existing Prompt Inspector. P0 traces the current GM prompt chunk lifecycle (included / pinned / truncated / evicted / empty / inactive) without changing prompt output, adding ledgers, semantic retrieval, Remote/Replay exposure, `TurnResult`, `statePatch`, or State Orchestrator wiring.

- **Context Engine P0 Inspector** — `contextInspectorCore.ts`: pure `buildContextInspectorReport` with chunk decision semantics (`included` / `included_pinned` / `truncated_by_budget` / `evicted_by_budget` / `skipped_inactive` / `skipped_empty`) sharing `applyPromptChunkBudgetRecords` with `evictPromptChunksByBudget`. `gmPromptBuilder.ts` attaches optional `contextInspector` to `PromptContextBreakdown`; Webview Inspector renders budget summary and per-chunk accounting. `scripts/test_context_inspector_core.js`, `scripts/test_context_inspector_integration.js`.
...
- **State Orchestrator SO1 Ledger Descriptor Inventory** — `stateOrchestratorDescriptorCore.ts`: pure metadata registry for nine ledger write surfaces (`game_state`, `discoveries`, `campaign_resources`, `settlement_layout`, `vehicle_state`, `world_state`, `npc_registry`, WI7 writeback, WI7b restore) with owner/phase/queue/backup/failure policy fields. `buildStateOrchestratorDescriptorReport` + `checkTurnLedgerDescriptorOrder` validate parity with read-only `TURN_LEDGER_PERSIST_ORDER`; no writes, no queue/order changes. `scripts/test_state_orchestrator_descriptor_core.js`. `npm test` **197/197**.
