...
- **重大 — 右タブを切替えても中身が出ない（再発）** — `#status-area` を flex 縦積みのまま 9 個の `.tab-pane` を並べていたため、active 以外の pane がレイアウト上ずれ、キャラクター/ワールド等が表示領域外に押し出されていた。**CSS Grid**（`auto 1fr` + 全 pane を同一セルに重ね）へ変更。`index.html` の inline `display:none` を削除し、`.active` クラスのみで表示制御。JS は `style.display` を触らず `classList` + `removeProperty('display')` に統一。
