...
<body>
  ...
  <div id="app">
    <div id="chat-area">
      <div id="chat-header">
        <span class="header-icon">⚔︁E/span>
        ...
        <div id="locale-wrap">
          ...
          <select id="locale-select" title="Language">
            ...
            <option value="zh-CN">简体中斁E/option>
            <option value="zh-TW">繁E��中斁E/option>
          </select>
        </div>
        <div id="img-gen-wrap">
        ...
        <div id="game-rules-wrap">
          <button id="game-rules-settings-btn" class="glass-btn" title="Game Rules">⚙︁E/button>
        </div>
      ...
      <div id="quick-reply-bar">
        ...
        <button class="qr-btn" id="qr-forcespeak" data-i18n="webview.quickReply.forceSpeak">🪁ESpeak as...</button>
        <button class="qr-btn" id="qr-questflow"  data-i18n="webview.quickReply.questFlow">🗺�E�EQuest Flow</button>
        <button class="qr-btn" id="qr-relations"  data-i18n="webview.quickReply.relations">🕸�E�ERelations</button>
      </div>
      <div id="input-area">
    ...
    <div id="status-area">
      ...
      <!-- ペイン 1: Adventure Status (既存�EスチE�Eタス・ダイス・BGM筁E -->
      <div id="pane-status" class="tab-pane active">
        ...
        <div class="status-block" id="status-block-skills">
          <span class="status-label" data-i18n="webview.status.skills">⚔︁ESkills</span>
      ...
      <div id="gallery-header">
        <span data-i18n="webview.gallery.header">🖼�E�EScene Gallery</span>
      </div>
      <div id="gallery"></div>
      ...
      <div id="theme-header">
        ...
        <div id="archive-suggest-banner" class="archive-suggest-banner" style="display: none;">
          ...
          <button id="archive-suggest-dismiss" type="button" title="Dismiss">ÁE/button>
        </div>
        ...
        <div id="summary-container" style="margin-top: 16px; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 12px;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; gap: 8px; flex-wrap: wrap;">
            ...
            <div style="display: flex; gap: 6px;">
              <button id="summarize-btn" data-i18n="webview.summary.generate" style="background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2); color: #fff; padding: 4px 8px; border-radius: 4px; cursor: pointer; font-size: 12px;">♻�E�EGenerate Summary</button>
  ...
  <aside id="remote-play-panel" class="remote-play-panel hidden" aria-hidden="true">
    <div class="img-gen-panel-header">
      ...
      <button type="button" id="remote-play-close" class="glass-btn" title="Close">✁E/button>
    </div>
  ...
  <aside id="img-gen-panel" class="img-gen-panel hidden" aria-hidden="true">
    <div class="img-gen-panel-header">
      ...
      <button type="button" id="img-gen-panel-close" class="glass-btn" data-i18n-title="webview.imageGen.close" title="Close">✁E/button>
    </div>
  ...
  <aside id="game-rules-panel" class="img-gen-panel hidden" aria-hidden="true">
    <div class="img-gen-panel-header">
      ...
      <button type="button" id="game-rules-panel-close" class="glass-btn" data-i18n-title="webview.gameRules.close" title="Close">✁E/button>
    </div>
    <div class="img-gen-panel-body">
...
/* ========= 世界観チE�Eマ別のグラチE�Eション背景�E�画像がなぁE��合�Eフォールバック�E�E========= */
...
function applyGameState(state, fullHistory) {
  ...
  if (state.entries && Array.isArray(state.entries)) {
    if (fullHistory) {
      // パネル再表示晁E 全履歴を新しい WebviewURI で置き換ぁE
      messageHistory = [];
      chatLog.innerHTML = '';
  ...
  // BGM の更新�E�EM ぁEbgm=トラチE��ID ぁEmood=ムード名を指定！E
  if (state.bgm) {
  ...
  // 効果音(SE)の再生�E�EM ぁEsfx=ID また�E [ID,...] を指定！E
  if (state.sfx) {
    const ids = Array.isArray(state.sfx) ? state.sfx : [state.sfx];
    ids.forEach(id => playSfx(id));
  }

  // 隠しダイス通知�E�EM ぁEhiddenDice に振ったダイスを記録�E�E
  if (Array.isArray(state.hiddenDice)) {
    state.hiddenDice.forEach(entry => {
      ...
      if (!seenHiddenDiceIds.has(entryId)) {
        ...
        const purposeText = entry.purpose ? `�E�E{entry.purpose}�E�` : '';
  ...
  // GM からのダイス要求！EiceRequest�E��E 自動ロール + 音で成否確誁E
...
function renderMessage(entry) {
  ...
  // 画像があれば表示 (セキュリチE��のためcreateElementを使用)
  if (entry.image) {
    ...
    regenBtn.onclick = () => {
      vscode.postMessage({
        ...
        mode: 'illustrious', // TODO: 汎用化できるならすめE
        entryId: entry.id
      });
...
function updateStatus(status) {
  ...
  if (dynamicContainer) {
    ...
    for (const [key, value] of Object.entries(status)) {
      if (value && typeof value === 'object' && 'current' in value && 'max' in value) {
        ...
        // affection めEreputation のような単一の数値 (0-100を想宁E の場吁E
  ...
  // 後方互換: 旧形式�E斁E���E condition も�E列として扱ぁE
...
function renderCheckpointUi() {
  ...
  if (checkpointMetas.length === 0) {
    ...
    for (const cp of checkpointMetas) {
      ...
      label.textContent = `${cp.label}  E${cp.turnLabel}`;
...
```javascript
// ===== BGM プレイヤー =====
// マニフェスト！Extension ぁEbgm.json を解決して webview URI 付きで送ってくる�E�E
// 吁Etrack: { id, uri, mood, description, loop, volume }
let bgmTracks = [];
let bgmCurrentId = null;
let bgmEnabled = true;
let bgmUserMuted = false;
let bgmBaseVolume = 0.5; // 0..1 ユーザー音釁E
let bgmAudioReady = false; // ユーザー操作で自動�E生が解禁されたぁE
...
function playBgmByMood(mood) {
  ...
  // mood フィールドが一致するトラチE��を探す（カンマ区刁E��褁E��対応！E
...
function playBgmById(id) {
  ...
  // 自動�E生がまだ解禁されてぁE��ぁE��合�E「曲名だけ表示」して征E��E
  if (!bgmAudioReady) {
...
function crossfadeTo(track) {
  ...
  // idle 側に新トラチE��をロードして再生
  bgmIdle.src = track.uri;
  bgmIdle.loop = track.loop !== false; // 既宁Eloop:true
  bgmIdle.volume = 0;
  const playPromise = bgmIdle.play();
  if (playPromise && playPromise.catch) {
    playPromise.catch(() => { /* 自動�E生ブロチE��時�E無要E*/ });
  }
...
// ===== 効果音(SE) =====
// マニフェスト！Extension ぁEsfx.json を解決して webview URI 付きで送る�E�E
// 吁Esound: { id, uri, description, volume }
let sfxSounds = [];
let sfxEnabled = true;
let sfxMuted = false;
...
function playSfx(id) {
  ...
  if (!s || !s.uri) return;
  // ワンショチE��: 毎回新しい Audio で重�E再生�E�EGM を止めなぁE��E
  ...
  a.play().catch(() => { /* 自動�E生ブロチE��時�E無要E*/ });
}

// Promise を返す牁E E音が実際に鳴ったかどぁE��検�Eできる
...
// GM からのダイス要求を処琁E E自動ロールし音の成否でフォールバックを判宁E
async function handleDiceRequest(req) {
  const notation = (req.notation || '').trim();
  const purposeText = req.purpose ? `�E�E{req.purpose}�E�` : '';
  ...
  // 音なしで自動ロール ↁE別送EplaySfxAsync で音を鳴らして成否を検�E
...
// Function()/eval を使わなぁE���Eな再帰下降パ�Eサー
...
function updateCharacterList(characters, activeId, partyIds) {
  ...
  // 選択状態を復允E��また�E Active キャラクターを選抁E
...
document.getElementById('char-upload-btn').addEventListener('click', () => {
  const id = charSelect.value;
  if (id === 'new') {
    // VSCode Webviewのalertは使えなぁE�EでSystemメチE��ージなどで警告すべきだが簡易的にreturn
    return;
  }
...
function startInlineEdit(msgDiv, entry, editBtn) {
  ...
  cancelBtn.textContent = T('webview.msg.editCancel') || '✁ECancel';
...
function renderTurnResult(turnResult) {
    ...
    if (turnIdDiv) {
        ...
        if (turnResult.turnId) {
            ...
            branchBtn.textContent = '⎁EBranch Timeline';
    ...
    if (diceLedgerDiv) {
        ...
        if (turnResult.diceLedger && turnResult.diceLedger.length > 0) {
            ...
            visibleLedger.forEach((entry) => {
                const row = document.createElement('div');
                row.className = 'inspector-item';
                let html = `<strong>${escapeHtml(entry.formula)}</strong> ➁E<span>${entry.total}</span>`;
                if (entry.reason) {
    ...
    if (statePatchDiv) {
        ...
        if (turnResult.statePatch && turnResult.statePatch.length > 0) {
            ...
            visiblePatches.forEach((patch) => {
                ...
                if (patch.op === 'add') { icon = '➁E; color = 'var(--text-success)'; }
                else if (patch.op === 'remove') { icon = '➁E; color = 'var(--text-danger)'; }

                row.innerHTML = `
                    <span title="${escapeHtml(patch.op)}">${icon}</span>
                    <code style="color:${color}">${escapeHtml(patch.path)}</code>
                    ${patch.value !== undefined ? `➁E<span class="patch-value">${escapeHtml(JSON.stringify(patch.value))}</span>` : ''}
                `;
...
function renderLorebookList(payload) {
    ...
    if (meta) {
        ...
        meta.textContent = typeof T === 'function'
            ...
            : `${writeFile}  E${count} entries (edits save here)`;
    }
    ...
    sorted.forEach((entry) => {
        ...
        if (isEditing) {
            ...
            card.innerHTML = `
                ...
                <div class="patch-value">${keys ? escapeHtml(keys) : ' E}</div>
...
function renderScenarioDirector(director) {
    ...
    setText('director-title', director.scenarioTitle || ' E);
...
function renderList(id, items) {
    ...
    if (!items || items.length === 0) {
        el.innerHTML = `<span class="empty-text"> E/span>`;
        return;
    }
    items.forEach((item) => {
        const row = document.createElement('div');
