...
- ✨ **Genesis Guide (Unreleased, UIプレビュー):** Start Hub の「世界を作りはじめる」から、世界観・遊び方・危険度・管理量・主人公作成方法・画像生成有無を選択肢だけで決める段階式ウィザード。有効になる主要システムと画像生成プロンプトをその場プレビュー。現状は Webview UI のみで、選択結果を `game_rules.json` へ実際に適用する host 側処理は未実装（次バージョンで対応予定）。
...
| Inspector（Debug Trace 込み） | Remote Play | Party Director |
|:---:|:---:|:---:|
...
自分で世界観から決めたい場合は、Start Hub 上部の **🌟 世界を作りはじめる**（Genesis Guide）から、選択肢だけで世界観・遊び方・危険度・管理量を選べます（Unreleased・UIプレビュー段階。設計: [`docs/RULES_PROFILE_ONBOARDING_DESIGN.md`](docs/RULES_PROFILE_ONBOARDING_DESIGN.md)）。
