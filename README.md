...
[![Version](https://img.shields.io/badge/version-1.77.15-blue.svg)](https://github.com/GGF1sh/LoreRelay/releases)
...
### 🌟 Start Hub & Genesis Guide — 迷わず始められる入口
...
<p align="center"><sub>初回起動時に最初に見る画面。「世界を作る」「お試しデモ」「キャラと話す」「主人公を作る」の4択だけを前面に出し、地図デモ・デバッグサンドボックス・手動作成・プリセットなどは折りたたみ式の「その他の遊び方・詳細設定」へ。GMプロバイダ・ComfyUI・TTSはすべて任意で、追加設定なしで今すぐ遊べます。</sub></p>
...
<p align="center"><sub>Genesis Guide（世界の道しるべ）— 世界観・遊び方・危険度・管理量を選ぶたびに、有効になるシステムと ComfyUI 用プロンプトがその場で組み上がっていきます。</sub></p>
...
> **版の正本:** `package.json`（現行 **1.52.0**）· [`CHANGELOG.md`](CHANGELOG.md) · [`docs/VERSION_TRUTH.md`](docs/VERSION_TRUTH.md) · タスク黒板は [`AI_ROADMAP.md`](AI_ROADMAP.md)
