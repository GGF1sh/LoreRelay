...
<p align="center"><strong>AIの物語を、消えない「ゲーム世界」にする。</strong><br />
チャット、キャラクター、世界地図、記憶、経済、画像生成を、ローカルのJSON状態としてひとつにつなぐAI RPGフロントエンドです。</p>
...
<p align="center"><img src="docs/assets/hero-ui.jpg" alt="LoreRelay — ランタンに照らされた酒場でAIゲームマスターが冒険を進行するイメージ" width="720" /></p>
...
<p align="center">
  ...
  <a href="docs/FIRST_SESSION.md"><strong>初回ガイド</strong></a>
</p>

<p align="center">
  <img src="docs/assets/screenshot-status.png" width="820" alt="LoreRelayの冒険ログ。GMの語り、選択肢、HP・MP・好感度、所持品、スキルを一画面に表示" />
</p>

LoreRelay はLLMそのものではなく、**好きなAIをゲームマスターとして接続する、ローカルファーストのUI兼状態管理レイヤー**です。AIの返答を読み捨てず、世界状態・履歴・ロア・画像をファイルへ保存するので、長いキャンペーンを継続して遊べます。
...
| RPGフロントエンド | 生きている世界 | 好きなAIと接続 |
|:---|:---|:---|
| 会話・選択肢・自由入力、HP/MP、所持品、スキル、ダイス、BGM/SE | 地域・派閥・NPC・交易・時間を永続化し、ターンごとにシミュレーション | ローカルエージェント、VS Code LM、手動コピペ、OpenRouterに対応 |
| **記憶と検証** | **画像と地図** | **拡張できる設計** |
| Lorebook、Memory、Turn Inspector、監査ジャーナル | ComfyUIシーン生成、羊皮紙地図、VLM Visual Memory | JSONベースのシナリオ・ルール・状態を自分で編集可能 |

<details>
<summary><strong>全機能一覧とバージョン別ハイライトを開く</strong></summary>
...
```mermaid
flowchart LR
    Player["プレイヤーの選択・自由入力"] --> GM["好きなAI / GM"]
    GM --> Turn["turn_result.json"]
    Turn --> Gate["検証・状態パッチ適用"]
...
冒頭のメイン画面を含め、以下はすべて実際の Webview を撮影したものです。
