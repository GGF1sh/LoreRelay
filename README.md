...
場面画像を会話ログに添えたり、仲間の発言量や関係を調整したり。キャラクターや世界の設定はロアブックで整理できます。[「この場面を絵にする」](docs/VISUAL_COMPOSER_V1.md)では公開済みの場面からプロンプトを編集・コピーし、Web版AIなどで作った画像を取り込んで採用できます。プロンプト作成・画像取込にAPIキーは不要です。自動画像生成には別途ComfyUIなどの準備が必要です。
