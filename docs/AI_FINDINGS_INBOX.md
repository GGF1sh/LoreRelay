...
| `CLAUDE-20260724-001` | Claude Opus 4.8 | `3daf963` | `src/gambitCombatCore.ts` L1487-1500: `mechanics_v1`モードでユニットが`normalAttackAbility`を持たない場合（例: 空のabilityカタログでセッション作成）、レガシー攻撃パス(L1043-1056)が`tu.hp`に与えたダメージを、tick末尾の`units[name].hp = mechanicsStates[name].hp`再同期が毎tick巻き戻す。全戦闘が全員HP満タンのままTimeoutで膠着する。実運用ではhostが常に`resources/combat-abilities/v1-reference-abilities.json`を渡すため未発現だが、カタログ読込失敗時のフォールバックや空カタログのテストで顕在化する。attacks/deathsイベントは出続けるためイベント消費側と最終状態が矛盾する点も悪質。 | P3 | - | (判定待ち) |
