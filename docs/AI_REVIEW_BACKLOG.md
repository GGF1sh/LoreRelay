...
| `MEDIA-M1` | Compatibility Gate + Profile Spine | Media/Runtime | Critical | **BLOCKED — CANONICAL_INSTALLER_NOT_PASS** (main `34fec19`; gates `235/235`) | `MEDIA-ARCHITECTURE-001` | implementation `046385f`; candidate `e229713`; independent verify `18d81f9`; exact installer BAT exited non-zero because live IDE extension directory was locked; restart IDE, rerun canonical BAT successfully, then perform human smoke A-D; do not start M2-M7 | Codex | Claude independent verify |
| **OTHERS** | | | | | | | | |
