...
**Last Reconciled:** 2026-07-10 JST
...
| `ANTIGRAVITY-RELAY-003` | Real-smoke recovery for short GM Skill trigger and pending request processing | UX/Tooling | High | **VERIFYING - REAL_SMOKE_PARTIAL_PASS_SUPERSEDED_BY_004** (impl `1e18d25`; verify `3e203f2`; main `4aff826`; smoke `a0f8426`) | `ANTIGRAVITY-RELAY-002` | real smoke proved core file round trip + import + continuation, but waiting row/timer and pending UX failed; repaired by `ANTIGRAVITY-RELAY-004` | Codex 5.5 High | Independent verify |
| `ANTIGRAVITY-RELAY-004` | Relay completion state and short-trigger UX clarity | UX/Tooling | High | **VERIFYING - REAL_SMOKE_READY** (impl `5103dc3`; verify `292f3d9`; main `c03c8d4`; smoke `27a5123`) | `ANTIGRAVITY-RELAY-003` | merged + automated post-merge smoke PASS (`232/232`) + Antigravity BAT install PASS; next gate: real human completion-state smoke with short right-side trigger | Codex 5.5 High | Independent verify |
| **D2 (Determinism)**| | | | | | | | |
...
| `KNOWLEDGE-OPS-001` | Operational AI lookup workflow for Symbol Registry / terminology / protocol pairing | Tooling/Docs | High | **DONE** (impl `e80292a`; repair `02efaf9`; audit `409868e`; verify `11db84a` overridden to repair-required; repair verify `2440a88`; FAST smoke PASS) | `SYMBOL-REGISTRY-001` | targeted `npm run knowledge -- <query>` workflow; side-aware protocol pairing; no production protocol changes | Codex 5.5 High | Grok 4.5 High |
| `MEDIA-ARCHITECTURE-001` | ComfyUI media profiles, Action Router, manual handoff, visualIdentity | Architecture/Media | High | **DESIGN_READY** (design `78c19eb`; M1 integrated separately) | - | accepted architecture; M2-M7 remain unstarted; M3a visualIdentity LOCAL only; HYBRID AI fill after M5 | Grok 4.5 High (design) | ChatGPT (integrator) |
| `INSTALLER-RELEASE-001` | Antigravity fallback aggregation repair and release identity | Tooling | High | **VERIFYING — REAL_INSTALL_PENDING** (impl `ee005c5`; verify `9ba4fe4`; FAST gates PASS) | - | main integrated; requires canonical BAT exit 0 and installed LoreRelay `1.78.0`; no installer run in integration lane | Codex | Claude independent verify |
| `MEDIA-M1` | Compatibility Gate + Profile Spine | Media/Runtime | Critical | **VERIFYING — INSTALLER_RECOVERY_PENDING** (main `34fec19`; gates `235/235`) | `INSTALLER-RELEASE-001` | implementation `046385f`; candidate `e229713`; independent verify `18d81f9`; after installer success at `1.78.0`, restart Antigravity and perform human smoke A-D; do not start M2-M7 | Codex | Claude independent verify |
| **OTHERS** | | | | | | | | |
