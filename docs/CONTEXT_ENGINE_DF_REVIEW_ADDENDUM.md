...
### Q3. Event-Participant preservation → **「今すぐ数行で直す」は誤り(裏取りで訂正)。durability の設計判断としてP1 Timeline Index initiative へ寄せる。今の実装介入は保留(D)**

> **⚠️ v1訂正(2026-07-04, Opus 4.8 が lifecycle を実コード追跡した結果)**: 初版で「Chronicle projection が npcIds を捨てる実バグ、数行で今直せる」と書いたが、**これは浅かった。裏取りで判断が変わった。**

確認した lifecycle:
- `WorldChangeEvent.npcIds` は存在する(worldEventLogCore.ts:23)が、`recentChanges` は **`MAX_RECENT_CHANGES=20` の FIFO ローリング窓**(worldEventLogCore.ts:41, `mergeRecentChanges`)。durable な長期履歴ではない。
- Chronicle は永続蓄積されず、`buildChronicleForWorkspace`(chronicleLoader.ts:40-45)が **毎回再計算**する。durable 入力は `state_journal.ndjson`(全ターン追記)のみ。npcIds 付き world-event は 20件窓 `recentChanges` からしか Chronicle に入らない。
- したがって `ChronicleEvent` に `npcIds` を足して `extractWorldChangeEvents` で運んでも、**救えるのは直近20件の world-event 参加者だけ**。窓から溢れた過去の参加者は既に durable ストアに無い。→ **Timeline Index(「Alice の生涯」)の目的には不十分な、ほぼ見た目だけの修正。安い修正は蜃気楼だった。**

**訂正後の決定(D: 今は実装介入しない、設計判断としてP1へ)**: 「参加者IDを durable に長期保持する」には、Chronicle 投影ではなく **journal 追記時点(`state_journal.ndjson`)、または専用の participant-bearing event ledger** に書く必要がある。これは3行修正ではなく、**どこに durable な participant-linked 履歴を置くかという Entity Timeline Index / Historical Event Backbone(§2A)の中核設計判断そのもの**。よって North Star へも「今すぐ小修正」へも入れず、**P1 Timeline Index initiative の最初の設計問い**として引き継ぐ。

**この訂正自体の教訓**: 「安い実バグ修正」に見えたものが、lifecycle(ローリング窓 vs 再計算 vs durable journal)を追うと設計判断だった。**"cheap fix" と主張する前に、そのデータが durable にどこに残るかを必ず追跡する。**
...
| ~~`ChronicleEvent`の`npcIds`消失修正(小規模)~~ → **訂正: participant durability は数行修正ではなく P1 設計判断**(Chronicleは20件窓から再計算、durableは`state_journal.ndjson`。§Q3訂正参照) | **RESERVE → P1 Timeline Index initiative の最初の問い** |
