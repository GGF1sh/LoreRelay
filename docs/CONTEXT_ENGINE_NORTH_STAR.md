# Context & Knowledge Engine — North-Star Spec (v0.3)

> ステータス: **北極星仕様(ひとまず確定)**。Architecture Owner(Opus 4.8)が [`CONTEXT_ENGINE_DESIGN_BRIEF.md`](CONTEXT_ENGINE_DESIGN_BRIEF.md) v0.2 §9 の Open Questions 10問に決定を下したもの。
> 起草: 2026-07-04, Opus 4.8。**この後 Grok(adversarial)/ Gemini(repo consistency)/ ChatGPT(systems critic)/ Fable 5(narrative UX)のレビューを前提とする。まだ実装しない。**
...
**根拠**: (a) 実測で actor ≤10、支配的クエリは「actor X が知る関連情報は?」= actor-first。(b) 独立JSONファイル台帳パターンに自然に乗る。(c) 派生索引方式なら伝播フェーズを塞がない。global-ledger+knower方式は 1万NPC 前提でのみ優位だが、それは分布外(§0)。

**可逆性**: LOAD-BEARING。ここを global-first に変えるとスキーマ・永続・全クエリが変わる。ただし actor≤10 が崩れない限り再検討不要。
...
| F5 | 100万Event | Event Ledger を worldTurn windowing(actorは≤10なので無関係) | window外参照でクラッシュしないか |
| F6 | 時間逆行/遡及編集(acquiredAt > now) | 負のelapsed → salience計算をクランプ | 未来取得で減衰が壊れないか |
| F7 | 矛盾Claim爆発(actorが無限にClaim蓄積) | per-actor Claim 上限(≤10記憶と整合)で古い低salienceを退避 | 上限到達時の退避が決定論か |
...
1. **Q1 の actor≤10 前提**: これが崩れる設計変更(大規模群衆NPC等)が将来来るなら Q1 は再検討要。→ **Gemini**: repo 全体で actor 上限を破る計画がないか確認せよ。
...
4. **Q10 全般**: → **Grok**: 上表を実際に壊し、特に F3(access≠knowledge)/F8(receipt)/F10(フロア)の invariant を破れるか試せ。
