...
| **D3** | Domain UI（World タブ）+ F7–F10 パネル | **完了** | **1.40.0** |
| **D4** | Commerce 境界 + 留守ドリフト | **一部完了** | ledger **1.39.1** · drift **1.39.3** · 市場ボーナス **未** |
| **D5** | Officers via Bond + Council | **完了** | bond **1.39.4** · council **1.39.5** |

**次のマイルストーン:** D4 残（`commerce` stat → `tickMarketRecovery` ボーナス）、または Wave 2 続き（F12 家史エピローグ／F11 ギルドマスター設計）。D3 UI 出荷後、次セッションで **Extension Development Host（F5）による手動確認**を推奨（本 doc の実装は compile + テストスイートのみで検証済み）。
...
### Phase D3: Domain UI **✅ v1.40.0**

| 項目 | 内容 |
|------|------|
| ファイル | `worldView.ts` · `webview/modules/85-world.js`（`renderDomainPanel` 他） · `webview/modules/70-game-rules.js` · `webview/index.html` · i18n 4 言語 · `domainBridge.pickDomainForWebview` 拡張（region 名・陳情詳細・action catalog） |
| 受け入れ | OFF 時パネル非表示 · 数値が `game_state.domain` と一致 · 行動チップ/裁定/派遣/采配は全て `insertChatText` でチャット挿入（直接適用ではない） **✅** |
| 内容 | 領地ヘッダー・7 ステータスバー・家臣一覧（派遣中表示）・月次行動チップ・F7 陳情裁定カード・F8 隣国（開示情報のみ）・F9 派遣中一覧+帰還報告+派遣フォーム・F10 合戦（ラウンド進行+采配ボタン） |
| 未検証 | Extension Development Host（F5）での実機確認は次セッション推奨（compile + テストスイートのみで検証済み） |
...
```
Domain Mode v1.40.0: D1→D5 + D3 UI + Wave2 F7-F10（謁見/隣国/派遣/合戦）エンジン+UI 完了。次 P1=D4 市場ボーナス or Wave2 F11/F12。F5 実機確認は次セッション推奨。正本 docs/DOMAIN_MODE_DESIGN.md §12・§20、docs/FABLE5_WAVE2_PROPOSALS_DESIGN.md。
```
...
**現状（v1.40.0）:** `enableDomainMode` + Core / Events / Time / Prompt（compact）/ Bond / Council / 留守ドリフト / allowlist hardening / `npm run domain:balance` / **D3 World タブ UI** / **Wave2 F7 謁見・F8 隣国・F9 派遣・F10 合戦（エンジン+UI 一式）** **完了**。**未:** D4 市場ボーナス · chronicle 専用 assert · replay export への domain pick · Wave2 F11/F12。
...
| turn merge authoritative (PR-A) | 1.39.9 | `test_domain_turn_merge_conflict.js` |
| F7 謁見 / F8 隣国 / F9 派遣 / F10 合戦（engine） | 1.39.10–1.39.13 | `test_domain_audience_core.js` 他 4 本 |
| **D3 Domain UI + F7–F10 パネル** | **1.40.0** | `check_i18n_keys.js` · `validate_webview_html_structure.js`（**F5 実機確認は未** — 次セッション推奨） |

### 20.2 次の作業

| 優先 | ID | 内容 | 主なファイル | 受け入れ | 想定 Ver |
|------|-----|------|--------------|----------|----------|
| P0 | F5 | **Extension Development Host での D3 UI 実機確認** — チップ/裁定/派遣/采配の挿入テキストを GM が正しく解釈できるか | 手動（VSCode F5） | 各ボタンでチャット挿入 → GM が対応する `domainOps` を生成 | 次セッション |
...
| P1 | F12 | Wave2 F12 家史エピローグ（箸休め枠） | `docs/FABLE5_WAVE2_PROPOSALS_DESIGN.md` §F12 | `epilogueCore.ts` + Replay Export テンプレ | 1.4x.x |
...
| P3 | E | Replay export に domain pick | `replayExportSanitizeCore.ts` | エクスポートに領地サマリのみ | 任意 |
| P3 | F11 | Wave2 F11 ギルドマスターモード設計（別 doc 先行） | `docs/GUILD_MODE_DESIGN.md`（未作成） | F9 Mission Core の guild 流用を意識した先行設計 | 1.45+ |
| 配布 | — | GitHub Release | tag `v1.40.0` | `package.json` と一致 VSIX | 準備完了 |

**推奨着手順:** **F5 実機確認**（挿入テキスト→GM解釈の実地検証）→ **B**（D4 市場ボーナス）→ Wave2 続き（**F12** 箸休め or **F11** 設計）。**C**/**D**/**E** は並行可。