...
| ID | 名称 | 状態 | 想定 Ver |
|----|------|------|----------|
| **F7** | 謁見の間（Audience Hall・陳情裁定） | **完了**（engine + UI） | v1.40.0 |
| **F8** | 隣国ライバル領主（Rival Lords tick） | **完了**（engine + UI） | v1.40.0 |
| **F9** | 主命・派遣（Officer Missions） | **完了**（engine + UI） | v1.40.0 |
| **F10** | 合戦リゾルバ（Mass Battle） | **完了**（engine + UI） | v1.40.0 |
| **F11** | ギルドマスターモード（→ G トラック候補） | 温め枠・別 doc 先行 | 1.45+ |
| **F12** | 家史エピローグ書き出し | ブリーフ済・未実装 | 箸休め枠 |

> F7–F10 のエンジンは v1.39.10–1.39.13 で個別出荷済み、D3 UI（1.40.0）で World タブに配線完了。実装時に **D6–D9** として Domain 表へ再登録してよい（F3 → LW1 合流と同じ前例、任意）。
