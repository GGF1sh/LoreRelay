...
|------|-----|
| `package.json` | **1.82.4** |
| live main | `git fetch origin` followed by `git rev-parse origin/main` |
| release truth | `package.json`, `package-lock.json`, four README badges, `CHANGELOG.md`, and `docs/VERSION_TRUTH.md` must agree |

Do not embed a supposed latest main commit in this handoff: committing the document
would immediately make that value stale. Before any integration or test play, read
the live remote tip and checked-out version directly:
...
## Integrated stabilization tracks in 1.82.4

- **Debug-sandbox deterministic fast path**: recognized direct and numbered
  quick-option commands remain local and deterministic, without unintended
  Relay/GM dispatch. Gameplay requests use the shared mutation boundary and
  Relay is restricted to gameplay-only authority.
- **Writer-lease race harness repair (test infrastructure)**: empty-workspace
  and stale-takeover contenders now overlap behind an explicit barrier while
  the winner remains live. This repaired a scheduling-dependent assertion; it
  did not change production writer-locking code.
- **Installer bootstrap Git fixture repair (test infrastructure)**: bootstrap
  tests now create a branch-hermetic local bare remote whose symbolic `HEAD`
  points to the exact fixture `main`. This did not change installer production
  behavior and does not mean the live installer has been refreshed.
- **Current-main UI/i18n work is preserved**: Relay labels wait for locale data,
  locale changes refresh the Relay toggle and Send button, and audited theme /
  Start Hub translations remain integrated.

The genre-aware events and economy-profile memo remains **DESIGN ONLY**. No
genre-aware event catalog, commodity-role abstraction, or economy profile was
implemented by this stabilization integration.
...
- [ ] Implement and independently verify collapsed Relay-banner recovery. The
      existing resize/static harness coverage does not prove recovery from a
      persisted collapsed height.
- [ ] Refresh the live installer only after the collapsed-banner repair is
      integrated and independently verified. The managed installer checkout and
      installed extension have not been refreshed by this integration.
- [ ] 上記①②を実際のVSCode（人間 or computer-use経由）で実機プレイし、Relay banner /
      Player Action Hub の human smoke を完了させる。静的 webview harness、Node tests、
      compile は real VS Code extension-host human smoke の代替ではない。
