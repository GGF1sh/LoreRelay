...
require('../scripts/run_live_extension_qa').runLifecycle({observeOnly:true,spawnHost:(exe,args,opts)=>{
...
},afterReopen:async({workspace})=>{browser=await chromium.connectOverCDP('http://127.0.0.1:9340');
 try{
 ...
 }finally{fs.writeFileSync(path.join(__dirname,'pacing-controls-final.json'),JSON.stringify(evidence,null,2));await browser.close();}
