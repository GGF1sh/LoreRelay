    ...
    child.stdin = new Writable({ write(chunk, _, done) { record.input += chunk.toString(); done(); }, final(done) {
        queueMicrotask(() => {
            if (args.includes('--help')) { child.stderr.write('--input-format --output-format --disable-slash-commands --agent --model'); child.emit('close', 0); }
                ...
                emit({ event: 'init', conversation_id: 's', init: { model: 'fixture', tools: mode === 'tools' ? ['run_command'] : [] } });
...
(async () => {
    ...
    loggedIn = true;
    for (mode of ['tools', 'failed-exit']) { client = make(); await client.initialize(); await assert.rejects(client.generate('fixture', () => {})); client.dispose(); }
    ...
    client = make(); await client.initialize(); await assert.rejects(client.generate('fixture', () => {}), /timeout/); client.dispose(); assert(kills > 0);
