...
import { adoptCartographyAsset, readCartographyImage, saveCartographyOverlay,
    loadCartographyAsset, saveCartographyMarker, cartographyAssetDiskRevision } from './cartographyAssetStore';

let pending: { token: string; workspace: string; worldKey: string; diskRevision: string;
...
export async function handleWorldMapAsset(message: Record<string, unknown>, deps: {
    ...
    try {
        ...
        if (message.action === 'import') {
            ...
            pending = { token: randomUUID(), workspace: deps.workspace, worldKey, diskRevision: cartographyAssetDiskRevision(deps.workspace, forge), image };
            ...
            if (!pending || pending.token !== message.token || pending.workspace !== deps.workspace
                || pending.worldKey !== worldKey || pending.diskRevision !== cartographyAssetDiskRevision(deps.workspace, forge)) {
