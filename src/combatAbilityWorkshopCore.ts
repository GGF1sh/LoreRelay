...
export function saveCustomAbility(library: CustomAbilityLibrary, ability: AbilityDefinition, statuses: readonly StatusDefinition[], reservedIds: readonly string[] = []): CustomAbilityLibrary {
    ...
    if (library.abilities.some(item => item.id === ability.id) || reservedIds.includes(ability.id)) throw new Error('DUPLICATE_ABILITY_ID');
...
export function replaceCustomAbility(library: CustomAbilityLibrary, ability: AbilityDefinition, statuses: readonly StatusDefinition[], reservedIds: readonly string[] = []): CustomAbilityLibrary {
    const withoutCurrent = { ...library, abilities: library.abilities.filter(item => item.id !== ability.id) };
    return saveCustomAbility(withoutCurrent, ability, statuses, reservedIds);
}
export function removeCustomAbility(library: CustomAbilityLibrary, id: string): CustomAbilityLibrary {
    return { ...library, abilities: library.abilities.filter(ability => ability.id !== id) };
}
export function importCustomAbilityLibrary(json: string, existing: CustomAbilityLibrary, statuses: readonly StatusDefinition[], reservedIds: readonly string[] = []): { library: CustomAbilityLibrary; error?: string } {
    try { const parsed = JSON.parse(json) as CustomAbilityLibrary; if (parsed.schemaVersion !== 'combat-ability-workshop-v1' || !Array.isArray(parsed.abilities)) throw new Error('INVALID_SCHEMA'); let next = emptyCustomAbilityLibrary(); for (const ability of parsed.abilities) next = saveCustomAbility(next, ability, statuses, reservedIds); return { library: next }; } catch (error) { return { library: existing, error: error instanceof Error ? error.message : 'INVALID_JSON' }; }
}
...
export function splitBuiltinAndCustom(fixture: AbilityFixtureDocument, custom: CustomAbilityLibrary) {
    return {
        builtin: fixture.abilities.map(ability => structuredClone(ability)),
        custom: custom.abilities.map(ability => structuredClone(ability)),
    };
}
